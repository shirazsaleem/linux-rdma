		    ...
		    u8 *mac_addr, u32 action)
{
	...
	case IRDMA_ARP_ADD:
		...
		err = irdma_alloc_rsrc(rf, rf->allocated_arps,
				       rf->arp_table_size, (u32 *)&arp_index,
				       &rf->next_arp_index);
		if (err)
			goto arp_err;
 ...
 * @num: not used
 */
static void irdma_free_qp(struct irdma_cqp_request *cqp_request, u32 num)
{
			    ...
			    struct irdma_cqp_request *cqp_request)
{
	...
	do {
		...
		irdma_debug(&rf->sc_dev, IRDMA_DEBUG_ERR,
			    "error cqp command 0x%x timed out", info->cqp_cmd);
		err_code = -ETIME;
		if (!rf->reset) {
			rf->reset = true;
			...
			u32 qp_num)
{
	...
	dma_free_coherent(irdma_hw_to_dev(rf->sc_dev.hw),
			  iwqp->q2_ctx_mem.size, iwqp->q2_ctx_mem.va,
			  iwqp->q2_ctx_mem.pa);
	iwqp->q2_ctx_mem.va = NULL;
	dma_free_coherent(irdma_hw_to_dev(rf->sc_dev.hw),
			  iwqp->kqp.dma_mem.size, iwqp->kqp.dma_mem.va,
			  iwqp->kqp.dma_mem.pa);
 ...
 * @unused: unused param
 */
static void irdma_ieq_ah_cb(struct irdma_cqp_request *cqp_request, u32 unused)
{
	struct irdma_sc_qp *qp = cqp_request->param;
 ...
 * @unused: unused param
 */
static void irdma_ilq_ah_cb(struct irdma_cqp_request *cqp_request, u32 unused)
{
...
/* TODO: Remove to VMWare files if needed */
