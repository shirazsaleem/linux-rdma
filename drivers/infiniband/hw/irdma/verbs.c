			      ...
			      struct ib_udata *udata)
{
	...
	props->max_mr_size = hw_attrs->max_hw_outbound_msg_size;
				...
				struct ib_udata *udata)
{
	...
	if (uk_attrs->hw_rev == IRDMA_GEN_1) {
		...
		uresp.max_hw_push_inline = uk_attrs->max_hw_push_inline;
 ...
 * @udata: user data
 */
static int irdma_alloc_pd(struct ib_pd *pd, struct ib_ucontext *context,
			  struct ib_udata *udata)
{
				...
				struct irdma_qp_init_info *info)
{
	...
	irdma_get_wqe_shift(uk_attrs, ukinfo->max_sq_frag_cnt + 1,
			    ukinfo->max_inline_data, &sqshift);
	status = irdma_get_sqdepth(uk_attrs, ukinfo->sq_size, sqshift, &sqdepth);
	if (status)
		return -ENOMEM;

	irdma_get_wqe_shift(uk_attrs, ukinfo->max_rq_frag_cnt, 0, &rqshift);
	status = irdma_get_rqdepth(uk_attrs, ukinfo->rq_size, rqshift, &rqdepth);
	if (status)
		return -ENOMEM;
	...
	mem->va = dma_alloc_coherent(irdma_hw_to_dev(iwdev->rf->sc_dev.hw),
				     ...
				     struct ib_udata *udata)
{
	...
	iwqp->q2_ctx_mem.size = ALIGN(IRDMA_Q2_BUF_SIZE + IRDMA_QP_CTX_SIZE,
				      256);
	iwqp->q2_ctx_mem.va = dma_alloc_coherent(irdma_hw_to_dev(dev->hw),
			 ...
			 int attr_mask, struct ib_udata *udata)
{
	...
	if (attr_mask & IB_QP_PATH_MTU) {
		...
		if (attr->path_mtu < IB_MTU_256 ||
			...
			dev_warn(to_device(dev), "Invalid MTU %d\n",
				 attr->path_mtu);
			return -EINVAL;
		}
...
static void irdma_free_cqbuf(struct work_struct *work)
{
	...
	dma_free_coherent(irdma_hw_to_dev(cq_buf->hw), cq_buf->kmem_buf.size,
			   ...
			   struct ib_udata *udata)
{
	...
	if (udata) {
		...
		if (ucontext->abi_ver < IRDMA_ABI_VER)
			return -ENOTSUPP;
		...
		kmem_buf.va = dma_alloc_coherent(irdma_hw_to_dev(dev->hw),
	...
	info.check_overflow = iwcq->sc_cq.check_overflow;
	...
	if (status) {
		kfree(cq_buf);
		irdma_debug(dev, IRDMA_DEBUG_ERR, "CQP-OP Resize CQ fail");
		return -EPROTO;
	}
				     ...
				     struct ib_udata *udata)
{
	...
	if (udata) {
		...
		if (rf->sc_dev.hw_attrs.uk_attrs.feature_flags &
		    IRDMA_FEATURE_CQ_RESIZE &&
		    ucontext->abi_ver >= IRDMA_ABI_VER) {
			spin_lock_irqsave(&ucontext->cq_reg_mem_list_lock, flags);
		...
		iwcq->kmem.va = dma_alloc_coherent(irdma_hw_to_dev(dev->hw),
						   iwcq->kmem.size,
		...
		iwcq->kmem_shadow.size = ALIGN(IRDMA_SHADOW_AREA_SIZE << 3,
					       64);
		iwcq->kmem_shadow.va = dma_alloc_coherent(irdma_hw_to_dev(dev->hw),
							  iwcq->kmem_shadow.size,
							  &iwcq->kmem_shadow.pa,
							  GFP_KERNEL);
 ...
 */
static int irdma_hw_alloc_mw(struct irdma_pci_f *rf, struct irdma_mr *iwmr)
{
...
static int irdma_attach_mcast(struct ib_qp *ibqp, union ib_gid *ibgid, u16 lid)
{
	...
	if (!mc_qht_elem) {
		...
		dma_mem_mc->size = ALIGN(sizeof(u64) * IRDMA_MAX_MGS_PER_CTX,
					 IRDMA_HW_PAGE_SIZE);
		dma_mem_mc->va = dma_alloc_coherent(irdma_hw_to_dev(&rf->hw),
						    dma_mem_mc->size,
						    &dma_mem_mc->pa,
						    GFP_KERNEL);
		if (!dma_mem_mc->va) {
...
error:
	...
	if (!mc_qht_elem->mc_grp_ctx.no_of_mgs) {
		...
		dma_free_coherent(irdma_hw_to_dev(&rf->hw),
