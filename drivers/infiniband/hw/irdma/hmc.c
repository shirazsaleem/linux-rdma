			...
			struct irdma_hmc_del_obj_info *info, bool reset)
{
	...
	for (i = 0; i < info->del_sd_cnt; i++) {
		...
		if (!mem || !mem->va)
			...
			dma_free_coherent(irdma_hw_to_dev(dev->hw), mem->size,
					  mem->va, mem->pa);
			mem->va = NULL;
		}
	}
						...
						u64 direct_mode_sz)
{
	...
	if (!sd_entry->valid) {
		...
		dma_mem.va = dma_alloc_coherent(irdma_hw_to_dev(hw),
		...
		if (type == IRDMA_SD_TYPE_PAGED) {
			...
			if (!vmem->va) {
				dma_free_coherent(irdma_hw_to_dev(hw),
						...
						struct irdma_dma_mem *rsrc_pg)
{
	...
	if (!pd_entry->valid) {
		if (rsrc_pg) {
			...
			page->va = dma_alloc_coherent(irdma_hw_to_dev(dev->hw),
						      page->size, &page->pa,
						      GFP_KERNEL);
			if (!page->va)
				return IRDMA_ERR_NO_MEMORY;
