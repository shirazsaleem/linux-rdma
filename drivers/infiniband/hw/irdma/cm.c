...
static void irdma_passive_open_err(struct irdma_cm_node *cm_node, bool reset)
{
	...
	cm_node->state = IRDMA_CM_STATE_CLOSED;
	irdma_debug(cm_node->dev, IRDMA_DEBUG_CM, "cm_node=%p state =%d\n",
		    cm_node, cm_node->state);
			   ...
			   u32 len)
{
	...
	*type = IRDMA_MPA_REQUEST_ACCEPT;
	...
	if (mpa_frame->rev > cm_node->mpa_frame_rev) {
		irdma_debug(cm_node->dev, IRDMA_DEBUG_ERR, "rev %d\n",
			    mpa_frame->rev);
		return -1;
	}
...
static void irdma_qp_disconnect(struct irdma_qp *iwqp)
{
	...
	if (iwqp->active_conn) {
		...
		if (iwqp->ietf_mem.va) {
			...
			dma_free_coherent(irdma_hw_to_dev(iwdev->rf->sc_dev.hw),
					  iwqp->ietf_mem.size,
					  iwqp->ietf_mem.va,
					  iwqp->ietf_mem.pa);
...
int irdma_accept(struct iw_cm_id *cm_id, struct iw_cm_conn_param *conn_param)
{
	...
	irdma_debug(cm_node->dev, IRDMA_DEBUG_CM, "Accept vlan_id=%d\n",
		    cm_node->vlan_id);
	...
	iwqp->ietf_mem.va = dma_alloc_coherent(irdma_hw_to_dev(dev->hw),
					       iwqp->ietf_mem.size,
	...
	if (IS_ERR(ibmr)) {
		dma_free_coherent(irdma_hw_to_dev(dev->hw),
	...
	list_for_each_entry (listen_node, &cm_core->listen_nodes, list) {
		...
		if (!list_empty(&listen_node->child_listen_list)) {
			irdma_qhash_ctrl(iwdev, listen_node, &nfo, ipaddr, ipv4,
				  ...
				  ipv4 ? 4 : 16)) {
			ret = irdma_manage_qhash(iwdev, &nfo,
