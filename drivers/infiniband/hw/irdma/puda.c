						   ...
						   u32 len)
{
	...
	if (!buf_mem.va) {
		irdma_debug(dev, IRDMA_DEBUG_PUDA, "error virt_mem for buf\n");
		return NULL;
	}

	buf = buf_mem.va;
	buf->mem.size = ALIGN(len, 1);
	buf->mem.va = dma_alloc_coherent(irdma_hw_to_dev(dev->hw),
					 buf->mem.size, &buf->mem.pa,
					 GFP_KERNEL);
	if (!buf->mem.va) {
		irdma_debug(dev, IRDMA_DEBUG_PUDA, "error dma_mem for buf\n");
		kfree(buf_mem.va);
		return NULL;
	}
...
static void irdma_puda_dele_buf(struct irdma_sc_dev *dev,
				struct irdma_puda_buf *buf)
{
	dma_free_coherent(irdma_hw_to_dev(dev->hw), buf->mem.size,
			  buf->mem.va, buf->mem.pa);
	buf->mem.va = NULL;
...
static enum irdma_status_code irdma_puda_qp_create(struct irdma_puda_rsrc *rsrc)
{
	...
	rsrc->qpmem.size = ALIGN((sq_size + rq_size + (IRDMA_SHADOW_AREA_SIZE << 3) + IRDMA_QP_CTX_SIZE),
				 IRDMA_HW_PAGE_SIZE);
	rsrc->qpmem.va = dma_alloc_coherent(irdma_hw_to_dev(rsrc->dev->hw),
	...
	if (ret) {
		dma_free_coherent(irdma_hw_to_dev(rsrc->dev->hw),
				  rsrc->qpmem.size, rsrc->qpmem.va,
				  rsrc->qpmem.pa);
		rsrc->qpmem.va = NULL;
		return ret;
	}
...
static enum irdma_status_code irdma_puda_cq_create(struct irdma_puda_rsrc *rsrc)
{
	...
	rsrc->cqmem.size = ALIGN(cqsize + sizeof(struct irdma_cq_shadow_area),
				 IRDMA_CQ0_ALIGNMENT);
	rsrc->cqmem.va = dma_alloc_coherent(irdma_hw_to_dev(dev->hw),
...
error:
	if (ret) {
		dma_free_coherent(irdma_hw_to_dev(dev->hw), rsrc->cqmem.size,
			  ...
			  bool reset)
{
	...
	case PUDA_QP_CREATED:
		...
		dma_free_coherent(irdma_hw_to_dev(dev->hw), rsrc->qpmem.size,
